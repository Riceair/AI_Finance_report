...
label=[6,7,9,10,15];  %5,7,9,10,12,15
...
for i=1:TV-1
    ...
    trainResult=ClassificationKNN.fit(train_data,train_ans,'NumNeighbors',2); %train_data 訓練資料陣列 train_ans 訓練資料答案
    the_predict=predict(trainResult,test_data);  %trainResult為訓練結果 test_data為訓練資料
    KNN_accuracy=length(find(the_predict == test_ans))/length(test_ans)*100; %算正確率
    the_accuracy(i,1)=KNN_accuracy;
    ...
    for k=1:length(the_predict) %統計return值
        if data(test_num-1+k,19)~=-100
            ...
            if the_predict(k)==1
                predict_return = predict_return + data(test_num-1+k,19);
