...
label=[6,7,9,10,12,15,17]; %6,7,9,10,12,15
...
for i=1:TV-1
    ...
    trainResult=fitcsvm(train_data,train_ans,'KernelFunction','rbf','OptimizeHyperparameters',{'BoxConstraint','KernelScale'},'HyperparameterOptimizationOptions',struct('ShowPlots',false));
    the_predict=predict(trainResult,test_data);  %trainResult為訓練結果 test_data為訓練資料
    SVM_accuracy=length(find(the_predict == test_ans))/length(test_ans)*100; %算正確率
    the_accuracy(i,1)=SVM_accuracy;
...
xlabel ('TV');
ylabel('return');
plot(TV,the_return(:,2),'g*-.','MarkerSize',12)
hold on
grid on %劃出格線
